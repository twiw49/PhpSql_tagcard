...
var symp_child_symp = function() {
  var url = "sql/medicalData/final_symptom.json";
...
var symp_disease_symp = function() {
  var url = "sql/medicalData/final_disease.json";
  $.getJSON(url, function(data) {
    for (var i = 0; i < data.length; i++) {
      if (data[i]['symptom'].length > 0) {
        for (var j = 0; j < data[i]['symptom'].length; j++) {
          var url_ = "sql/medicalData/final_symptom.json";
          $.ajax({
            url: url_,
            async: false,
            type: "GET",
    ...
    for (var i = 0; i < data.length; i++) {
      if (data[i]['disease'].length > 0) {
        for (var j = 0; j < data[i]['disease'].length; j++) {
          var url_ = "sql/medicalData/final_disease.json";
          $.ajax({
            url: url_,
            async: false,
            type: "GET",
